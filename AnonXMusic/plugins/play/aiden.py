...
@app.on_message(filters.command(["مطور السورس", "سلوم","ليثون"], ""))
async def dev(client: Client, message: Message):
     bot_username = client.me.username
     user = await client.get_chat(7004732448)
...
@app.on_message(filters.command(["قلب سلوم", "سوسي","سوسو"], ""))
async def dev(client: Client, message: Message):
     bot_username = client.me.username
     user = await client.get_chat(7004732448)
     name = user.first_name
     ...
     await message.reply_photo(
     photo=photo,
     caption=f"<b>• معلومات قلب سلوم</b>\n\n<b>• 𝖭𝖺𝗆𝖾 : {name}</b>\n<b>• 𝖴𝗌𝖾 : @{username}</b>\n<b>• 𝖡𝗂𝗈 : {bio}</b>",
     reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"{name}", user_id=f"{user_id}")]]))
     try:
       os.remove(photo)
     except:
